...
ax.set_title("US WEEKLY diff(1) and sell-in diff(1)")
plt.show()
...
maxlag = 12
df_test = pd.DataFrame()
df_test['y'] = scaled_ext[[y_label]].diff().diff()[2:-1]
...
ax.set_title("consumer_price_index diff(1) and sell-in diff(1)")
plt.show()
...
ax.set_title("airline_ticket_price_index diff(1) and sell-in diff(1)")
plt.show()
