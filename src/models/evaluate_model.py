...
def evaluate_and_store_performance(model_type, ticker_symbol, y_true, y_pred):
    ...
    file_path = f"{config['paths']['output_path']}/model_performance_daily.csv"
...
def daily_model_evaluation(model_type=None, ticker_symbol=None):
    ...
    if ticker_symbol:
        ...
        # print(current_train_df.tail())

        # print(historical_forecasts_df.head())
    ...
    for ticker_symbol in historical_forecasts_df["STOCK"].unique():
        ...
        for model_type in available_models:
            ...
            try:
                ...
                logger.warning(f"\nNeed to wait for the next day after making the first forecast in order to calculate the performance. Comeback tomorrow.")
                raise ValueError(f"\nNeed to wait for the next day after making the first forecast in order to calculate the performance. Comeback tomorrow.")


            # logger.debug(f"Actual value: {latest_value}. Predicted Value: {predicted_value}")

            # Evaluate and store yesterday's performance
            evaluate_and_store_performance(model_type, ticker_symbol, latest_value, predicted_value)

daily_model_evaluation()#model_type='xgb', ticker_symbol='bova11')
